function [estimated_Sysem_RLS,covariance_RLS,error] = RLS5(dx,dy,N,T,N0)
epsilon=10^-8;
%step 0, estimating Theta_0 using Least Square and set P
U = Unbyp5(dx(1:N0),dy(1:N0));
Theta = pinv(U'*U)*U'*dy(1:N0);
a_hat = [1 Theta(1) Theta(2) Theta(3) Theta(4) Theta(5)];
b_hat = [Theta(6) Theta(7) Theta(8) Theta(9) Theta(10)];
P = 1000*eye(10);
...
%Step 1 & 2: 
    ...
    u_t = [-dy(N0);-dy(N0-1);-dy(N0-2);-dy(N0-3);-dy(N0-4);dx(N0+1);dx(N0);dx(N0-1);dx(N0-2);dx(N0-3)]; 
%Step 3:
